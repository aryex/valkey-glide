...
class TestPubSub:
    ...
    async def test_pubsub_exact_happy_path(
        ...
        async with async_pubsub_test_clients(
            request,
            cluster_mode,
            subscription_method,
            channels={channel},
    ...
    async def test_pubsub_exact_happy_path_many_channels(
        ...
        channels_and_messages: Dict[str, str] = {
            f"{shard_prefix}channel_{i}": f"message_{i}" for i in range(NUM_CHANNELS)
        }
        ...
        try:
            ...
            for _ in range(max_samples):
                current_sync_ts = await poll_for_timestamp_change(
                    previous_sync_ts, timeout_s=3.0
                )
                sampled_interval_ms = current_sync_ts - previous_sync_ts
...
class TestPubSubLargeMessage:
    ...
    async def test_pubsub_exact_max_size_message(
        ...
        try:
            ...
            message = self.make_message("1", self.MESSAGE_SIZE)
            message2 = self.make_message("2", self.MESSAGE_SIZE)
    ...
    async def test_pubsub_exact_max_size_message_callback(
        ...
        try:
            ...
            message = self.make_message("0", self.MESSAGE_SIZE)
