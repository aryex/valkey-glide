...
class TestSyncPubSubLargeMessage:
    """
    Tests for publishing and receiving maximum size large messages in PUBSUB.

    These tests require the server's client-output-buffer-limit for pubsub to be
    disabled (set to 0 0 0) so that large messages are not rejected by the
    ...
    Note: The protocol message limit is 512MB. This proved to be too large for CI runners
    so we moved to 256MB.
    """

    # Size of the message used in test (MB).
    MESSAGE_SIZE = 256

    def make_message(self, seed: str, size_mb: int):
        """Helper to create a message of a specific size."""
        return seed * size_mb * 1024 * 1024

    @pytest.fixture(autouse=True)
    def configure_buffer_limit(self, request):
        """Disable pubsub output buffer limit for max-size message tests."""
        cluster_client = create_sync_client(request, cluster_mode=True)
        standalone_client = create_sync_client(request, cluster_mode=False)
        try:
            cluster_client.config_set({"client-output-buffer-limit": "pubsub 0 0 0"})
            standalone_client.config_set({"client-output-buffer-limit": "pubsub 0 0 0"})
            yield
        finally:
            cluster_client.config_set(
                {"client-output-buffer-limit": "pubsub 33554432 8388608 60"}
            )
            standalone_client.config_set(
                {"client-output-buffer-limit": "pubsub 33554432 8388608 60"}
            )
    ...
    def test_sync_pubsub_exact_max_size_message(
        ...
        """
        Tests publishing and receiving maximum size large messages in PUBSUB.

        Verifies that large messages can be published
        and received correctly in both cluster and standalone modes.
        """

        channel = get_random_string(10)
        message = self.make_message("1", MESSAGE_SIZE)
        message2 = self.make_message("2", MESSAGE_SIZE)
    ...
    def test_sync_pubsub_sharded_max_size_message(
        ...
        """
        Tests publishing and receiving large messages in sharded PUBSUB.
    ...
    def test_sync_pubsub_exact_max_size_message_callback(
        ...
        """
        Tests publishing and receiving large messages in exact PUBSUB
        with callback method.
        """

        channel = get_random_string(10)
        message = self.make_message("0", MESSAGE_SIZE)
